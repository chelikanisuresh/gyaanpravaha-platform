...
export default function PasswordPage() {
  ...
  // Reset form when child switches
  const handleSelect = (id: string) => {
    setSelectedId(id); setPw(''); setConfirm(''); setMsg(''); setErr('')
  }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { setMsg(`Password updated! Share the new password with ${child?.full_name}.`); setPw(''); setConfirm('') }
    ...
    <ParentSidebarLayout parentName={parentName}>
      <div style={{ maxWidth:'520px', padding:'28px 28px 60px' }}>
        ...
        <p style={{ fontFamily:'var(--font-body)', fontSize:'14px', color:'#6B7280', marginBottom:'20px' }}>
          {child ? `Change ${child.full_name}'s login password` : "Change your child's login password"}
        </p>
        ...
        <div style={{ background:'white', borderRadius:'14px', border:'1px solid #E5E7EB', padding:'22px' }}>
          ...
          <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
            ...
            <button type="submit" disabled={loading||!child} style={{ background:'#2D6A4F', color:'white', fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'14px', padding:'10px 22px', borderRadius:'8px', border:'none', cursor:(loading||!child)?'not-allowed':'pointer', alignSelf:'flex-start', opacity:(loading||!child)?0.7:1 }}>
