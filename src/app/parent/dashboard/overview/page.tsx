    ...
    <div style={{ background:'white', borderRadius:'14px', border:'1px solid #E5E7EB', overflow:'hidden' }}>
      ...
      {open && (
        <form onSubmit={submit} style={{ padding:'16px 18px', display:'flex', flexDirection:'column', gap:'12px' }}>
          {[{id:'np',label:'New password',val:pw,set:setPw,ph:'Minimum 8 characters',type:'password',min:8},{id:'cp',label:'Confirm password',val:confirm,set:setConfirm,ph:'Re-enter password',type:'password'}].map(({id,label,val,set,ph,type,min})=>(
            <div key={id}>
              ...
              <input type={type} value={val} onChange={e=>set(e.target.value)} required minLength={min} placeholder={ph} style={{ width:'100%', padding:'10px 14px', fontFamily:'var(--font-body)', fontSize:'14px', color:'#1F2937', background:'white', border:'1.5px solid #E5E7EB', borderRadius:'8px', outline:'none' }}/>
            </div>
          ))}
      ...
      <div style={{ maxWidth:'820px', padding:'28px 28px 60px' }}>
        ...
        {child && (
          <div style={{ marginBottom:'20px' }}>
            ...
            <PasswordCard childName={child.full_name} childId={child.id}/>
          </div>
        )}
            ...
            <div key={s.name} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 18px', borderBottom:i<arr.length-1?'1px solid #F9FAFB':'none' }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:s.available?'5px':0 }}>
                  ...
                  {s.available
                    ? <p style={{ fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'12px', color:'#2D6A4F' }}>{s.completed}/{s.total}</p>
                    : <span style={{ background:'#F3F4F6', color:'#9CA3AF', fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'10px', padding:'2px 8px', borderRadius:'20px' }}>Coming soon</span>}
                </div>
                {s.available && (
                  <div style={{ height:'5px', background:'#E5E7EB', borderRadius:'3px', overflow:'hidden' }}>
                    <div style={{ height:'100%', width:`${Math.round(((s.completed||0)/(s.total||1))*100)}%`, background:'#2D6A4F', borderRadius:'3px' }}/>
                  </div>
                )}
              </div>
            </div>
