...
function ScoreBar({ score, max=100 }: { score:number; max?:number }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
      ...
      <span style={{ fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'12px', color, minWidth:'36px', textAlign:'right' }}>{score}/{max}</span>
    </div>
  )
}
  ...
  return (
    <ParentSidebarLayout parentName={parentName}>
      <div style={{ maxWidth:'820px', padding:'28px 28px 60px' }}>
        ...
        <p style={{ fontFamily:'var(--font-body)', fontSize:'14px', color:'#6B7280', marginBottom:'24px' }}>{childName} · {completed} of 8 chapters completed · {avg}% average score</p>
                    ...
                    <div style={{ display:'flex', gap:'16px', flexWrap:'wrap', marginBottom: ch.score!==null ? '8px' : 0 }}>
                      {[{l:'Sections',v:`${ch.sectionsRead}/7`},{l:'Time',v:`${ch.timeSpent}m`},{l:'Attempts',v:String(ch.attempts)}].map(({l,v}) => (
                        <div key={l}>
                          ...
                          <p style={{ fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'13px', color:'#374151' }}>{v}</p>
                        </div>
                      ))}
                    </div>
