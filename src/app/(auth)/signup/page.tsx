    ...
    <div style={{ minHeight: '100vh', background: 'var(--green-pale)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      ...
      <div className="card" style={{ width: '100%', maxWidth: '480px', padding: '40px 36px' }}>
        ...
        <form onSubmit={handleDetailsSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          ...
          <div>
            <label className="label">Child's gender</label>
            <div style={{ display: 'flex', gap: '10px' }}>
              {['male', 'female'].map(g => (
                <button key={g} type="button"
                  onClick={() => update('childGender', g)}
                  style={{ flex: 1, padding: '10px', borderRadius: '8px', cursor: 'pointer', fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '14px',
                    border: form.childGender === g ? '2px solid var(--green-dark)' : '1.5px solid #E5E7EB',
                    background: form.childGender === g ? 'var(--green-pale)' : 'white',
                    color: form.childGender === g ? 'var(--green-dark)' : '#6B7280',
                  }}>
                  {g === 'male' ? '👦 Male' : '👧 Female'}
                </button>
              ))}
