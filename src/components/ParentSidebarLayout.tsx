    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '38px', height: '38px', borderRadius: '50%', background: '#6366F1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: '15px', color: 'white', flexShrink: 0 }}>
            {parentName[0]}
          </div>
          <div>
            <p style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: '14px', color: 'white', lineHeight: 1, marginBottom: '2px' }}>{parentName}</p>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'rgba(255,255,255,0.5)' }}>Parent</p>
          </div>
        </div>
      </div>
        ...
        {NAV_ITEMS.map(({ href, label, emoji }) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontFamily: 'var(--font-heading)', fontWeight: isActive ? 700 : 600, fontSize: '14px', color: isActive ? '#74C69D' : 'rgba(255,255,255,0.75)' }}>
                {label}
              </span>
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F8FAF9' }}>
      <style>{`
        @media (max-width: 768px) {
          ...
          .parent-bottom-nav { display: flex !important; }
          .parent-main { margin-left: 0 !important; padding-bottom: 72px !important; }
        }
        @media (min-width: 769px) {
          .parent-sidebar { display: flex !important; }
          .parent-bottom-nav { display: none !important; }
        }
      `}</style>
      ...
      {/* Main content */}
      <div className="parent-main" style={{ flex: 1, marginLeft: '220px', minHeight: '100vh' }}>
        {children}
      </div>

      {/* Mobile bottom nav */}
      <div className="parent-bottom-nav" style={{
        display: 'none',
