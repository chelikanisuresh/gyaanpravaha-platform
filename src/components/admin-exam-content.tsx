...
const CHAPTER_NAMES: Record<string, Record<number, string>> = {
  ...
  history: { 1:'The Vedas — Our Sacred Heritage',2:'Essence of Hinduism',3:'The Great Preachers',4:'The Preamble',5:'India Lives in Villages',6:'The Power of Determination' },
  geo:     { 1:'Our Earth in the Solar System',2:'The Earth as a Globe — 1 (Movements)',3:'The Earth as a Globe — 2 (Imaginary Lines)',4:'Landforms',5:'Representation of Geographical Features',6:'Agriculture in India and World',7:'North America' },
  sanskrit:{ 1:'Prarthana (Prayer)',2:'Vivekananda (Vivekanandah)',3:'Sanchalana Geetam (March Song)',4:'Sanskritabhasha Grihe Grihe',5:'Sankhyah (Numbers 21-40)',6:'Sandhi (Combination of Letters)',7:'Bhutakalah (Past Tense)',8:'Sambhashanam (Conversation)' },
  ict:     { 1:'File Management — Organization of Data',2:'Artificial Intelligence',3:'Introduction to HTML',4:'HTML — Formatting a Web Page',5:'Creating Tables in HTML' },
  marathi: { 1:'Bharatmata',2:'Majha Anubhav',3:'Paaus Aala! Paaus Aala!',4:'Mahiti Gheuya',5:'Sugaraniche Gharte',6:'He Khare Khare Vhave',7:'Udyanat Bhetlela Vidyarthi',8:'Kundache Sahas',9:'Ghar',10:'Babanch Patra',11:'Minucha Jalpravaas',12:'Chandravarchi Shala',13:'Mothi Aai',14:'Appajinche Chaturya',15:'Holi Aali Holi',16:'Mukya Pranyachi Kaifiyat',17:'Panpoi' },
  rapid:   { 1:'Monday Morning',2:'Rajam and Mani',3:"Swami's Grandmother",4:'What is a Tail?',5:"Father's Room",6:'A Friend in Need',7:'A New Arrival',8:'Before the Examinations',9:'School Breaks Up',10:"The Coachman's Son",11:"In Father's Presence",12:'Broken Panes',13:'The M.C.C.',14:"Granny Shoves Her Ignorance",15:'Before the Match',16:'Swami Disappears',17:'The Day of the Match',18:'The Return',19:'Parting Present' },
}
...
// Key: "subject__term"
type ConfigMap = Record<string, ExamConfig>

function configKey(subject: string, term: string) { return `${subject}__${term}` }

export default function ExamContent() {
  // configs keyed by "subject__term"
  ...
  useEffect(() => {
    const load = async () => {
      ...
      ;(data ?? []).forEach((row: any) => {
        map[configKey(row.subject, row.term)] = row
      })
      setConfigs(map)
      setLoading(false)
    }
  ...
  // Get config for current subject+term (or empty default)
  const getConfig = (subjectId: string): ExamConfig => {
    const key = configKey(subjectId, selectedTerm)
    return configs[key] ?? { subject: subjectId, term: selectedTerm, chapter_ids: [], is_active: false, duration_mins: 60 }
  }

  const toggleChapter = (subjectId: string, chapterId: number) => {
    const key  = configKey(subjectId, selectedTerm)
    const curr = getConfig(subjectId)
    ...
    setConfigs(prev => ({ ...prev, [key]: { ...curr, chapter_ids: ids } }))
  }

  const saveSubject = async (subjectId: string) => {
    setSaving(subjectId)
    const supabase = createClient()
    const cfg = getConfig(subjectId)
    const { error } = await supabase.from('exam_config').upsert(
      { subject: cfg.subject, term: selectedTerm, chapter_ids: cfg.chapter_ids, is_active: cfg.is_active, duration_mins: cfg.duration_mins },
      { onConflict: 'subject,term' }
    )
    if (error) {
  ...
  const toggleActive = async (subjectId: string) => {
    const key    = configKey(subjectId, selectedTerm)
    const curr   = getConfig(subjectId)
    const newVal = !curr.is_active
    setConfigs(prev => ({ ...prev, [key]: { ...curr, is_active: newVal } }))
    const supabase = createClient()
    const { error } = await supabase.from('exam_config').upsert(
      { subject: curr.subject, term: selectedTerm, chapter_ids: curr.chapter_ids, is_active: newVal, duration_mins: curr.duration_mins },
      { onConflict: 'subject,term' }
    )
    if (error) alert(`Toggle failed: ${error.message}`)
  }

  const activeLiveCount = SUBJECTS.filter(s => getConfig(s.id).is_active).length
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '20px', marginBottom: '28px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '14px', color: '#64748B' }}>
            Configure chapters per test type. {activeLiveCount > 0 ? `${activeLiveCount} subject${activeLiveCount > 1 ? 's' : ''} live for ${selectedTerm}.` : `No subjects live for ${selectedTerm}.`}
          </p>
        </div>
        <div>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: '#64748B', marginBottom: '8px' }}>Viewing test type</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 20px' }}>
                  ...
                  <button onClick={() => setExpandedSubj(isExpanded ? null : subject.id)}
                    ...
                    <p style={{ fontFamily: 'var(--font-body)', fontSize: '12px', color: '#94A3B8', marginTop: '2px' }}>
                      {selected > 0 ? `${selected} chapter${selected > 1 ? 's' : ''} selected for ${selectedTerm}` : `No chapters selected for ${selectedTerm}`} · {isExpanded ? 'collapse' : 'expand'}
                    </p>
                  </button>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexShrink: 0 }}>
                    ...
                    <input type="number" min={10} max={180} value={config.duration_mins}
                      onChange={e => {
                        const key = configKey(subject.id, selectedTerm)
                        setConfigs(prev => ({ ...prev, [key]: { ...config, duration_mins: Number(e.target.value) } }))
                      }}
                      ...
                      {Array.from({ length: subject.chapters }, (_, i) => i + 1).map(chId => {
                        ...
                        return (
                          <button key={chId} onClick={() => toggleChapter(subject.id, chId)}
                            ...
                            <div style={{ width: '20px', height: '20px', borderRadius: '6px', border: `2px solid ${checked ? subject.color : '#CBD5E1'}`, background: checked ? subject.color : 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              {checked && <span style={{ color: 'white', fontSize: '12px', fontWeight: 900 }}>✓</span>}
                            </div>
                            <span style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: checked ? subject.color : '#374151', fontWeight: checked ? 600 : 400, flex: 1 }}>
                              {names[chId] ?? `Chapter ${chId}`}
                            </span>
