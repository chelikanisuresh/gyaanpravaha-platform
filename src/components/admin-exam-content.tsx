...
export default function ExamContent() {
  ...
  const saveSubject = async (subjectId: string) => {
    ...
    const { error } = await supabase.from('exam_config')
      .upsert({ subject: configs[subjectId].subject, chapter_ids: configs[subjectId].chapter_ids, is_active: configs[subjectId].is_active, duration_mins: configs[subjectId].duration_mins, term: selectedTerm }, { onConflict: 'subject,term' })
    if (error) {
  ...
  const toggleActive = async (subjectId: string) => {
    ...
    await supabase.from('exam_config')
      .upsert({ subject: configs[subjectId].subject, chapter_ids: configs[subjectId].chapter_ids, is_active: newVal, duration_mins: configs[subjectId].duration_mins, term: selectedTerm }, { onConflict: 'subject,term' })
  }
